...
void WaterHeatCtrl::step(const tb_WaterHeatCtrl_In *rtu_WaterHeatCtrl_In, tb_WaterHeatCtrl_Out *rty_WaterHeatCtrl_Out)
{
  ...
  WaterHeatControllerMDLOBJ1.step(&rtu_WaterHeatCtrl_In->Ss_tank_size, &rtu_WaterHeatCtrl_In->Cs_tank_down_temp, &rtu_WaterHeatCtrl_In->Cs_tank_up_temp,
    &rtu_WaterHeatCtrl_In->Cs_pump_evap_temp, &rtu_WaterHeatCtrl_In->Cs_pump_xhst_temp, &rtu_WaterHeatCtrl_In->Ss_op_mode, &rtu_WaterHeatCtrl_In->Ns_pers_nb,
    &rtb_Cs_heat_pump_rot_spd_sp);
  ...
  if (rtu_WaterHeatCtrl_In->Bs_v40_sp_simu_ena) {
    rty_WaterHeatCtrl_Out->Cs_v40_sp = rtu_WaterHeatCtrl_In->Cs_v40_sp_simu;
  } else {
    rty_WaterHeatCtrl_Out->Cs_v40_sp = rtb_Cs_v40_sp;
  }
  ...
  rty_WaterHeatCtrl_Out->Cs_v40_rat = WaterHeatCtrl_DW.Cs_v40_rat;
}
