...
//(temporaire)	#include "UsbHostCDC.h"		// Pour accès aux Paramètres & Fonctions USB_Host_CDC
	...
	#if defined(NB_BLUETOOTH_BUF_DEF_SIZE) && (NB_BLUETOOTH_BUF_DEF_SIZE > 0)
		#include "BluetoothUART.h"	// Pour accès aux Paramètres & fonction bluetooth
	#endif // NB_BLUETOOTH_BUF_DEF_SIZE
...
#if defined(NB_BLUETOOTH_BUF_DEF_SIZE) && (NB_BLUETOOTH_BUF_DEF_SIZE > 0)	// cf. "ModbusSlaveConf.h"
	UART_COM_MAKE_STRUCT_BUF_VAR_ARRAY(tBluetoothBuf, BLE_UART_MAX_RX_SIZE, BLE_UART_MAX_TX_SIZE, BluetoothBuf, NB_BLUETOOTH_BUF_DEF_SIZE);
#endif // NB_COM_MODBUS
...
//	{ UartComModbus,	UART_MODBUS_USER,	MX_UART5_Init,			UART_COM_STRUCT_BUF_INFOS(ModbusSlaveBuf[0]),	ModbusSlaveRxHandler,	MODBUS_SLAVE_UART_USER_PARAMS,	UART_COM_GPIO_TX_NONE,	MODBUS_SLAVE_EOF_RX,	MODBUS_SLAVE_EOB_RX,	MODBUS_SLAVE_REPLY_TO,	MODBUS_SLAVE_NO_TX_INIT,	MODBUS_SLAVE_NO_TX_FRAME,	MODBUS_SLAVE_NO_TX_BYTE,	MODBUS_SLAVE_NO_RX_TO,	MODBUS_SLAVE_MIN_RX_SIZE,	MODBUS_SLAVE_MIN_TX_SIZE,	MODBUS_SLAVE_MAX_RX_BLOC_SZ,	MODBUS_SLAVE_DEF_REPLY_SZ,	MODBUS_SLAVE_ERR_RX_FRAME,	MODBUS_SLAVE_ERR_TX_RETRY,	MODBUS_SLAVE_ERR_TX_FRAME,	MODBUS_SLAVE_INIT_DO_LOAD_FLAGS,	UART_COM_CLASS_TX_DMA_RX_IT },	// ModbusSlave User sur Uart5 en TX_DMA & RX_IT
	{ UartComModbus,	UART_MODBUS_USER,	MX_UART5_Init,			UART_COM_STRUCT_BUF_INFOS(ModbusSlaveBuf[0]),	ModbusSlaveRxHandler,	MODBUS_SLAVE_UART_USER_PARAMS,	UART_COM_GPIO_TX_NONE,	MODBUS_SLAVE_EOF_RX,	MODBUS_SLAVE_EOB_RX,	MODBUS_SLAVE_REPLY_TO,	MODBUS_SLAVE_NO_TX_INIT,	MODBUS_SLAVE_NO_TX_FRAME,	MODBUS_SLAVE_NO_TX_BYTE,	MODBUS_SLAVE_NO_RX_TO,	MODBUS_SLAVE_MIN_RX_SIZE,	MODBUS_SLAVE_MIN_TX_SIZE,	MODBUS_SLAVE_MAX_RX_BLOC_SZ,	MODBUS_SLAVE_DEF_REPLY_SZ,	MODBUS_SLAVE_ERR_RX_FRAME,	MODBUS_SLAVE_ERR_TX_RETRY,	MODBUS_SLAVE_ERR_TX_FRAME,	MODBUS_SLAVE_INIT_DO_LOAD_FLAGS,	UART_COM_CLASS_TX_DMA_RX_IDLE_DMA },	// ModbusSlave User sur Uart5 en TX_DMA & RX_IDLE_DMA
...
#endif // NB_EMBRACO_INVERTER_BUF_DEF_SIZE > 0

#if defined(NB_BLUETOOTH_BUF_DEF_SIZE) && (NB_BLUETOOTH_BUF_DEF_SIZE > 0)	// cf. "EmbracoInverterConf.h"
	{ UartComCustom,	UART_COM_BLE,		MX_UART4_Init,			UART_COM_STRUCT_BUF_INFOS(BluetoothBuf[0]), BLE_UART_RX_FN_HANDLER, BLUETOOTH_SYS_MANAGER,		UART_COM_GPIO_TX_NONE,	BLE_UART_EOF_RX,		BLE_UART_EOB_RX,		BLE_UART_REPLY_TO,		BLE_UART_NO_TX_INIT,		BLE_UART_NO_TX_FRAME,		BLE_UART_NO_TX_BYTE,		BLE_UART_NO_RX_TO,		BLE_UART_MIN_RX_SIZE,		BLE_UART_MIN_TX_SIZE,		BLE_UART_MAX_RX_SIZE,			BLE_UART_DEF_REPLY_SZ,		BLE_UART_ERR_RX_FRAME,		BLE_UART_ERR_TX_RETRY,		BLE_UART_ERR_TX_FRAME,		BLE_UART_INIT_DO_LOAD_FLAGS, 		UART_COM_CLASS_TX_IT_RX_IT }, // EmbracoInverter sur Uart4
#endif // NB_EMBRACO_INVERTER_BUF_DEF_SIZE > 0
...
#ifdef UART_COM_ENABLE_BLE	// cf. "UartComConf.h"
	InitBluetoothMST();	// Initialise @ MST les bluetooth sur UART
#endif // UART_COM_ENABLE_BLE
}
