...
1) Dans CubeMX, aller dans "ProjectManager -> Project" :
    ...
    * A la 1° Configuration, s'assurer d'avoir déjà bien sélectionné le "Firmware Package" le plus récent parmi ceux compatibles !
  

2) Dans "ProjectManager -> Code Generator" s'assurer des Paramètres suivants :
...
## Etape III : Configurer "BaseDeTemps.h" 
1) Décommenter (ci-dessous) "BDT_SUPPORT_ALIVE_RT" si vous souhaitez utiliser le Compteur de RunTime natif "AliveRunTime1s" de cette Librairie

## Etape III : Configurer le Projet 

**Dans le nouveau Projet**

1) Ajouter l'Include du .h dans le "main.c" et "stm32*_it.c" (par exemple parmi les "USER CODE * Includes") :
```
...
3)  Ajouter le Handler @ 1ms dans une Interruption :   
...
5) Ajouter le Dossier "BaseDeTemps" à l' "IncludePath", pour toutes les Configs de Build.  
...
6) Vérifier que le Dossier "BaseDeTemps" ne soit "Exclude From Build" d'aucune des Configs.
