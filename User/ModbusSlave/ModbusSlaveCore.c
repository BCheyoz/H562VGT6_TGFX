...
#ifdef MODBUS_SLAVE_SUPPORT_READ_HOLDING_REGISTERS
...
#ifdef MODBUS_SLAVE_SUPPORT_WRITE_MULTIPLE_REGISTERS
...
void InitModbusSlaveStruct(tModbusSlaveParams* pStruct, uint16_t srcId, uint16_t slaveAdr) //, tModbusItem *baseTable)
{
	...
	ModbusSlaveFillMemory((void*)pStruct, 0, sizeof(tModbusSlaveParams));
...
#if !defined(DISABLE_MODBUS_SLAVE_SUPPORT) && defined(MODBUS_SLAVE_SUPPORT_READ_HOLDING_REGISTERS)
...
static int HandleModbusFunctionReadHoldingRegisters(tRxTxBufInfo* pRxTxBI, tModbusSlaveParams* pModbusSlave, int *retValue)
{
	...
	if(pRxTxBI->RxBuf.nbBytes < (2 +4 +2)) { *retValue = MODBUS_SLAVE_ERR_ILL_DATA; return MODBUS_SLAVE_FRAME_WAS_EXCEPTION; }		// Taille Trame Insuffisante
...
// inutile	*retValue = 0; // Par défaut : rien à Répondre
	...
	if(124 < nbRegister) { nbRegister = 124; } // Max 124 registres 16bits pour rentrer dans 256 Bytes
...
#if !defined(DISABLE_MODBUS_SLAVE_SUPPORT) && defined(MODBUS_SLAVE_SUPPORT_WRITE_MULTIPLE_REGISTERS)
...
// inutile	*retValue = 0; // Par défaut : rien à  Répondre
	...
	if(nbRx != (7 + (nbRegister <<1) +2)) { *retValue = MODBUS_SLAVE_ERR_ILL_ADR; return MODBUS_SLAVE_FRAME_WAS_EXCEPTION; } // Retourne une Exception
	...
	uint16_t IndexTrame = 7; // 7 sur MODBUS_FCT_WRITE
...
{
	uint8_t i;
	for (i = 0; i < sizeof(accessPswdTable)/sizeof(uint16_t); i++)
	{
		if(accessPswdTable[i] == Pswd)
			return i;
	}
