...
2) Ajouter la référence à votre/vos Variable(s) Publique(s) de Sortie dans la zone "USER CODE Public_Vars" du même .h :
    ```
	extern I2CCM_Pres_ABP2_ExtData	mPresABP2;	// Exemple d'une Variable Publique "mPresABP2"
    ```
3) Ajouter votre/vos Variable(s) Publique(s) de Sortie dans la zone "USER CODE Public_Vars" du .c de votre Gestionnaire :
    ```
	I2CCM_Pres_ABP2_ExtData mPresABP2 = {0};  	// Exemple d'une Variable Publique "mPresABP2"
    ```
4) Ajouter la ligne de Configuration à votre "InitTable",  dans la zone "USER CODE InitTable" de ce même .c :  
    ...
    * le 3° Paramètre identifie le Type de Capteur à charger. Il y a 2 méthodes possibles :
       - Soit demander le chargement d'un Type en particulier, en spécifiant son Indice, par exemple "I2cDevPresType_ABP2_Adr00", ou "I2cDevPresType_ABP2_Adr01",
    ...
    * le 4° Paramètre est le nom de la Fonction à appeler pour Charger notre module : "i2cCM_PressureABP2_Init"
    * le 5° Paramètre est un Identifiant de Type dans la Famille "I2CCM_DevicePressureFamily", par exemple "I2cDevPresFamilyABP2"
    ...
    * le 7° Paramètre est le Pointeur vers notre Variable Publique de sortie, de type "I2CCM_Pres_ABP2_ExtData"  
    
    ```
    // Exemple 1 : Groupe "Internal Pressure", pas de Multiplexeur I2C, Capteur par défaut, Famille ABP2, VariableInterne #0, Sortie sur "mPresABP2" :
    	{ I2cIntDevGrpPressure,	I2CCM_CHANNEL_NONE, I2CCM_LOAD_DEFAULT_DEVICE,	i2cCM_PressureABP2_Init, I2cDevPresFamilyABP2,	0, &mPresABP2 },
    
    // Exemple 2 : Groupe "External Pressure", pas de Multiplexeur I2C, Capteur par défaut n°3, Famille ABP2, VariableInterne #0, Sortie sur "mXtPresABP2" :
     	{ I2cExtDevGrpPressure,	I2CCM_CHANNEL_NONE, I2CCM_LOAD_DEFAULT_DEVICE3,	i2cCM_PressureABP2_Init, I2cDevPresFamilyABP2,	0, &mXtPresABP2 },
    
    // Exemple 3 : Groupe "System Pressure", pas de Multiplexeur I2C, Capteur par défaut n°2, Famille ABP2, VariableInterne #1, Sortie sur "mSysPresABP2" :
     	{ I2cSysDevGrpPressure,	I2CCM_CHANNEL_NONE, I2CCM_LOAD_DEFAULT_DEVICE2,	i2cCM_PressureABP2_Init, I2cDevPresFamilyABP2,	1, &mSysPresABP2 },
    ```
