...
static int32_t st7789v_write_reg(ST7789V_Object_t *pObj, volatile uint8_t *Reg, uint32_t Length);
static int32_t st7789v_send_data(ST7789V_Object_t *pObj, uint8_t *pData, uint32_t Length);
static int32_t st7789v_recv_data(ST7789V_Object_t *pObj, uint8_t *pData, uint32_t Length);
...
int32_t ST7789V_Init(ST7789V_Object_t *pObj, ST7789V_InitParams_t* pInitParams)
{
	...
	else if (pObj->IsInitialized == 0)
	{
		/* Sleep In Command */
		reg = ST7789V_SPLIN;
		ret += st7789v_write_reg(pObj, &reg, 1);
		/* Wait for 10ms */
		...
		ret += st7789v_write_reg(pObj, &reg, 1);
		ret += st7789v_send_data(pObj, (uint8_t *)parameter, 2);
		...
		if(pInitParams->SwapRB)
		{
			...
			parameter[0] = 0x0C;
			ret += st7789v_write_reg(pObj, &reg, 1);
			ret += st7789v_send_data(pObj, (uint8_t *)parameter, 1);
		}
		...
		parameter[4] = 0x33;
		ret += st7789v_write_reg(pObj, &reg, 1);
		ret += st7789v_send_data(pObj, (uint8_t *)parameter, 5);
...
{
	int32_t ret;
	uint8_t volatile data[4] = {ST7789V_READ_DISPLAY_ID, 0, 0, 0 };

	if(st7789v_recv_data(pObj, (uint8_t *)data, 4) != ST7789V_OK)
	{
		ret = ST7789V_ERROR;
	}
	else
...
{
	uint8_t volatile reg = ST7789V_DISPLAY_ON;

	return st7789v_write_reg(pObj, &reg, 1);
}
...
{
	uint8_t volatile reg = ST7789V_DISPLAY_OFF;
...
int32_t ST7789V_SetOrientation(ST7789V_Object_t *pObj, uint32_t Orientation)
{
	...
	if(ret == ST7789V_OK)
	{
		...
		case ST7789V_ORIENTATION_LANDSCAPE_ROT180:
			ST7789VCtx.Width  = pObj->YSize;
			ST7789VCtx.Height = pObj->XSize;
			break;
		default:
		case ST7789V_ORIENTATION_PORTRAIT_ROT180:
		case ST7789V_ORIENTATION_PORTRAIT:
			ST7789VCtx.Width  = pObj->XSize;
			ST7789VCtx.Height = pObj->YSize;
			break;
		}
...
int32_t ST7789V_SetDisplayWindow(ST7789V_Object_t *pObj, uint32_t Xpos, uint32_t Ypos, uint32_t Width, uint32_t Height)
{
	...
	uint8_t volatile reg = ST7789V_CASET;
	...
	ret += st7789v_send_data(pObj, (uint8_t *)parameter, 4);
