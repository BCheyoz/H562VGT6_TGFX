...
2) Ajouter la référence à votre/vos Variable(s) Publique(s) de Sortie dans la zone "USER CODE Public_Vars" du même .h :
    ```
	extern I2CCM_Pres_HSC_ExtData	mPresHSC;	// Exemple d'une Variable Publique "mPresHSC"
    ```
3) Ajouter votre/vos Variable(s) Publique(s) de Sortie dans la zone "USER CODE Public_Vars" du .c de votre Gestionnaire :
    ```
	I2CCM_Pres_HSC_ExtData mPresHSC = {0};  	// Exemple d'une Variable Publique "mPresHSC"
    ```
4) Ajouter la ligne de Configuration à votre "InitTable",  dans la zone "USER CODE InitTable" de ce même .c :  
    ...
    * le 3° Paramètre identifie le Type de Capteur à charger. Il y a 2 méthodes possibles :
       - Soit demander le chargement d'un Type en particulier, en spécifiant son Indice, par exemple "I2cDevPresType_HSC_Adr00", ou "I2cDevPresType_HSC_Adr01",
    ...
    * le 4° Paramètre est le nom de la Fonction à appeler pour Charger notre module : "i2cCM_PressureHSC_Init"
    * le 5° Paramètre est un Identifiant de Type dans la Famille "I2CCM_DevicePressureFamily", par exemple "I2cDevPresFamilyHSC"
    ...
    * le 7° Paramètre est le Pointeur vers notre Variable Publique de sortie, de type "I2CCM_Pres_HSC_ExtData"  
    
    ```
    // Exemple 1 : Groupe "Internal Pressure", pas de Multiplexeur I2C, Capteur par défaut, Famille HSC, VariableInterne #0, Sortie sur "mPresHSC" :
    	{ I2cIntDevGrpPressure,	I2CCM_CHANNEL_NONE, I2CCM_LOAD_DEFAULT_DEVICE,	i2cCM_PressureHSC_Init, I2cDevPresFamilyHSC,	0, &mPresHSC },
    
    // Exemple 2 : Groupe "External Pressure", pas de Multiplexeur I2C, Capteur par défaut n°3, Famille HSC, VariableInterne #0, Sortie sur "mXtPresHSC" :
     	{ I2cExtDevGrpPressure,	I2CCM_CHANNEL_NONE, I2CCM_LOAD_DEFAULT_DEVICE3,	i2cCM_PressureHSC_Init, I2cDevPresFamilyHSC,	0, &mXtPresHSC },
    
    // Exemple 3 : Groupe "System Pressure", pas de Multiplexeur I2C, Capteur par défaut n°2, Famille HSC, VariableInterne #1, Sortie sur "mSysPresHSC" :
     	{ I2cSysDevGrpPressure,	I2CCM_CHANNEL_NONE, I2CCM_LOAD_DEFAULT_DEVICE2,	i2cCM_PressureHSC_Init, I2cDevPresFamilyHSC,	1, &mSysPresHSC },
    ```

## Etape II : Configurer le Projet 
**Dans le nouveau Projet**

1) Ajouter le Dossier "I2c_Press_HSC" à l' "IncludePath" pour toutes les Configs de Build.  
  (Attention : CubeIDE a l'habitude de stocker les chemins relatifs au Workspace, et non au Dossier)    
 il est souvent préférable d'utiliser la formulation "../User/I2c_Press_HSC" (sans les guillemets)

Info : Pour vérifier, sélectionner "Properties" du Projet -> "C/C++ Build" -> "Settings" -> "Tool Settings" -> "MCU GCC Compiler" -> "Include paths".

2) Vérifier que le Dossier "I2c_Press_HSC" ne soit "Exclude From Build" d'aucune des Configs.  
